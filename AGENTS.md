...
```
- 生成文件名自动包含 `package.json` 中版本号的 `.vsix` 文件
- 包含所有必要的源文件和资源
- 文件大小约 180KB
...
### 当前 Runtime API 对接状态（undo / retry / revert）

这些能力已经由 TUI Runtime API 暴露，GUI 必须调用服务端端点，不能只修改 webview 的显示状态：

| 功能 | Runtime API | GUI 对接 |
|------|-------------|----------|
| 撤销对话轮次 | `POST /v1/threads/{id}/undo` | `CodeWhaleApiClient.undoThreadTurn()` |
| 撤销轮次文件改动 | `POST /v1/threads/{id}/patch-undo` | `CodeWhaleApiClient.patchUndoThreadTurn()` |
| 重试最后一轮 | `POST /v1/threads/{id}/retry` | `CodeWhaleApiClient.retryThreadTurn()` |
| 列出快照 | `GET /v1/snapshots` | `CodeWhaleApiClient.listSnapshots()` |
| 恢复快照 | `POST /v1/snapshots/{id}/restore` | `CodeWhaleApiClient.restoreSnapshot()` |

`GET /v1/runtime/info` 是运行时契约的事实来源。客户端应读取
`runtime_api_version`、`codewhale_version`、`capabilities` 和 `experimental`；
`version` 只保留为旧客户端兼容别名。模型选择器应从 `GET /v1/models`
读取当前 provider 的 catalog，不能在 webview 中维护固定模型列表。
